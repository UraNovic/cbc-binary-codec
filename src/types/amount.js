...
const DROPS_PER_cbc = new Decimal('1e6');
const MAX_NETWORK_DROPS = new Decimal('1e17');
const MIN_cbc = new Decimal('1e-6')
const MAX_cbc = MAX_NETWORK_DROPS.dividedBy(DROPS_PER_cbc);
...
Native values must be described in drops, a million of which equal one cbc.
...
const parsers = {
  string(str) {
    ...
    return [new Decimal(str).dividedBy(DROPS_PER_cbc), Currency.cbc];
  },
  object(object) {
...
const Amount = makeClass({
  Amount(value, currency, issuer) {
    ...
    this.currency = currency || Currency.cbc;
  ...
  statics: {
    ...
    fromParser(parser) {
      ...
      const cbcValue = drops.dividedBy(DROPS_PER_cbc);
      return new this(cbcValue, Currency.cbc);
    }
  },
  assertValueIsValid() {
    ...
    if (!this.isZero()) {
      if (this.isNative()) {
        ...
        if (abs.lt(MIN_cbc) || abs.gt(MAX_cbc)) {
          // value is in cbc scale, but show the value in canonical json form
          raiseIllegalAmountError(this.value.times(DROPS_PER_cbc))
        }
      } else {
  ...
  valueString() {
    return (this.isNative() ? this.value.times(DROPS_PER_cbc) : this.value)
            .toString();
  },
  toBytesSink(sink) {
